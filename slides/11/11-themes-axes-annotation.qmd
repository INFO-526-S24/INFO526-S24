...
list_of_files <- dir_ls(path = here::here("slides/11/data"), regexp = "Foreign Connected PAC")
...
pac <- pac |>
  janitor::clean_names() |>
  mutate(
    year = str_remove(year, "./INFO526-F23/slides/11/data/Foreign Connected PACs, "),
...
::: task
Open `ae-11` and implement the improvements you came up with in Part 1.
:::

## Revisit Tucson AQI

::: task
Recreate the following visualization, in Part 2 of `ae-11`. This picks up where you left off in `ae-09`.
:::
...
tuc_2022 <- read_csv(
  here::here("slides/11/", "data/tucson/ad_aqi_tracker_data-2022.csv"),
  na = c(".", "")
) |>
  janitor::clean_names() |>
...
```{r}
#| include: false
sf_files <- fs::dir_ls(here::here("slides/11/", "data/san-francisco"))
```
...
::: task
Highlight years using **gghighlight** in Part 3 of `ae-11`.
:::
